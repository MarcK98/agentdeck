...
// the team lead can check a comment the owner left without waiting for the poll feed.
server.tool(
  "trello_read",
  "Team lead only: READ the Trello board right now — returns the current cards (each with its status list, url, key, `ref`, `desc` — the card's full description — and any `attachments` (files/links the owner pasted on the card, shown with a 📎)) and recent comments the owner left. Use this to read a ticket's description/details, attachments, or check a comment on demand, instead of waiting for the automatic heartbeat feed. A card with a null `key` is an untracked card the owner made by hand — pass its `ref` to trello_write (as `card_ref`) to comment/move/archive it. Optional `card_key` filters comments to one card (the same stable key you pass to trello_sync); `limit` caps how many recent comments to return (default 20).",
  {
      ...
      if (!j.ok) {
        ...
        const cards = (j.cards || [])
          .map((c) => {
            ...
            // Attachments (files/links the owner pasted on the card) — one line each.
...
server.tool(
  "trello_attach",
  "Team lead only: attach a file or a link to a Trello card. Identify the card by `card_key` (a tracked task) or `card_ref` (id/shortLink/URL — for cards the owner made by hand). Provide either `path` (an ABSOLUTE path to a local file in the project workspace — uploaded to the card) or `url` (a link stored on the card), and an optional `name` label. Use this to put a screenshot, report, log, exported doc, or reference link on the relevant card.",
  {
    card_key: z.string().optional(),
    card_ref: z.string().optional(),
...
server.tool(
  "vpn",
  "Control the Tunnelblick VPN on the host (macOS). action: 'status' (default), 'connect', 'disconnect', or 'list'. `config` is the Tunnelblick configuration name (e.g. 'CorpVPN'); omit to use the default. Use this to bring the VPN up when a channel needs it to reach its backend/database/services — typically connect, do the work, then disconnect if desired. States: CONNECTED = up, EXITING = down.",
  {
