...
| a relay (local, self-hosted, or the hosted AgentDeck one) | the mobile app always pairs through a relay — the daemon dials out to it, phones dial in. Relay source lives in the private `agentdeck-saas` repo |
...
the daemon through a relay (default `:8820`) — point it at the hosted AgentDeck relay,
or run your own compatible one (source in the private `agentdeck-saas` repo), and set
`SPAWN_RELAY_URL` / `SPAWN_RELAY_DAEMON_KEY` so the daemon dials out to it.
...
packages/mobile   agentdeck-mobile — Expo client, paired to the daemon via a relay.
src/              compatibility shims only (keep `node src/index.js` working for the
                  legacy Discord bridge until its hard-cut).
```

The relay, marketing site, and billing infra live in the private `agentdeck-saas`
repo (not this one) — this repo is the open-source daemon/desktop/mobile product.
...
```json
{
  "my-app": "/Users/you/Documents/some-other-folder",
  "123456789012345678": "/Users/you/Documents/mapped-by-channel-id"
}
```
